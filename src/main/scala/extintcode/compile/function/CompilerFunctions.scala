...
object CompilerFunctions {
  ...
  def call(imports: ImportTable, runtime: CompilerRuntime, func: (FunctionEntry, ValType), statement: Boolean, params: LangExpression*): (List[AssemblyText], List[AssemblyData]) = {
    ...
    if (statement) runtime.startExpressionSection()
    ...
    BuiltinHeader.specialFunctionCode(func, function.signature.indices.map(params(_).value(runtime)): _*) match {
      case Some(x) =>
        text.addAll(x)
        if (statement) runtime.endExpressionSection()
      case None =>
        for (i <- function.signature.indices) {
